const fs = require("fs");
const http = require("http");
const path = require("path");
const crypto = require("crypto");
const pcsclite = require("pcsclite");
const { WebSocketServer } = require("ws");

const HOST = "127.0.0.1";
const PORT = Number(process.env.PORT || 25585);
const WS_PATH = "/ws";
const INDEX_PATH = path.join(__dirname, "public", "index.html");

const pcsc = pcsclite();
const IS_WINDOWS = process.platform === "win32";
const readerBusy = new Map();
const wsClients = new Set();
const WS_OPEN = 1;

function bufToHex(buf) {
  return buf
    ? buf.toString("hex").toUpperCase().replace(/(..)/g, "$1 ").trim()
    : "";
}

function bufToHexCompact(buf) {
  return buf ? buf.toString("hex").toUpperCase() : "";
}

function isSwOk(resp) {
  if (!resp || resp.length < 2) return false;
  const sw1 = resp[resp.length - 2];
  const sw2 = resp[resp.length - 1];
  return sw1 === 0x90 && sw2 === 0x00;
}

function swToString(resp) {
  if (!resp || resp.length < 2) return "----";
  const sw1 = resp[resp.length - 2].toString(16).padStart(2, "0");
  const sw2 = resp[resp.length - 1].toString(16).padStart(2, "0");
  return `${sw1}${sw2}`.toUpperCase();
}

function logResp(label, data) {
  console.log(`[apdu] ${label} -> ${bufToHex(data)} | SW=${swToString(data)}`);
}

function sha256Hex(value) {
  return crypto.createHash("sha256").update(value).digest("hex").toUpperCase();
}

function detectCardMode(readerName, atrHexCompact) {
  if (atrHexCompact === "3B8F80018031806549544A344C120FFF82900085") {
    return "CIE_CONTACTLESS";
  }
  if (atrHexCompact === "3B8B80010031C16408923354009000F3") {
    return "CNS_CONTACTLESS";
  }
  if (atrHexCompact === "3BFF1800008131FE45006B05052000012101434E5310318079") {
    return "CNS_CONTACT";
  }
  if (readerName.includes("Contactless")) return "UNKNOWN_CONTACTLESS";
  return "UNKNOWN_CONTACT";
}

function isResetCardError(err) {
  if (!err) return false;
  const msg = String(err.message || err);
  return msg.includes("0x80100068") || msg.includes("SCARD_W_RESET_CARD");
}

function connectReader(reader) {
  return new Promise((resolve, reject) => {
    const shareMode = reader.SCARD_SHARE_EXCLUSIVE ?? reader.SCARD_SHARE_SHARED;
    reader.connect({ share_mode: shareMode }, (err, protocol) => {
      if (err) return reject(err);
      resolve(protocol);
    });
  });
}

function disconnectReader(reader) {
  return new Promise((resolve) => {
    reader.disconnect(reader.SCARD_LEAVE_CARD, () => resolve());
  });
}

function rawTransmit(reader, protocol, apdu, label) {
  return new Promise((resolve, reject) => {
    reader.transmit(apdu, 0xff, protocol, (err, data) => {
      if (err) return reject(new Error(`${label}: ${err.message}`));
      resolve(data);
    });
  });
}

async function transmit(reader, protocolRef, apdu, label) {
  try {
    return await rawTransmit(reader, protocolRef.value, apdu, label);
  } catch (err) {
    if (!(IS_WINDOWS && isResetCardError(err))) throw err;

    console.log(`[pcsc] ${label}: Windows reset detected, reconnecting...`);

    await disconnectReader(reader);
    protocolRef.value = await connectReader(reader);

    return rawTransmit(reader, protocolRef.value, apdu, `${label} (retry)`);
  }
}

async function tx(reader, protocolRef, apdu, label) {
  const data = await transmit(reader, protocolRef, apdu, label);
  logResp(label, data);
  return data;
}

async function tryReadCieNis(reader, protocolRef) {
  const CIE_SELECT_IAS = Buffer.from([
    0x00, 0xa4, 0x04, 0x0c, 0x0d, 0xa0, 0x00, 0x00, 0x00, 0x30, 0x80, 0x00,
    0x00, 0x00, 0x09, 0x81, 0x60, 0x01,
  ]);

  const CIE_SELECT_DF_ID_SERVIZI = Buffer.from([
    0x00, 0xa4, 0x04, 0x0c, 0x06, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x39,
  ]);

  const CIE_READ_NIS = Buffer.from([0x00, 0xb0, 0x81, 0x00, 0x0c]);

  let response;

  response = await tx(reader, protocolRef, CIE_SELECT_IAS, "CIE SELECT IAS");
  if (isSwOk(response)) {
    response = await tx(
      reader,
      protocolRef,
      CIE_SELECT_DF_ID_SERVIZI,
      "CIE SELECT DF.ID_Servizi after IAS",
    );
    if (isSwOk(response)) {
      response = await tx(
        reader,
        protocolRef,
        CIE_READ_NIS,
        "CIE READ NIS after IAS",
      );
      if (isSwOk(response)) {
        const data = response.slice(0, -2);
        const ascii = String.fromCharCode(...data);
        return /^[0-9]{8,20}$/.test(ascii) ? ascii : Array.from(data).join("");
      }
    }
  }

  response = await tx(
    reader,
    protocolRef,
    CIE_SELECT_DF_ID_SERVIZI,
    "CIE SELECT DF.ID_Servizi direct",
  );
  if (!isSwOk(response))
    throw new Error(`CIE direct DF.ID_Servizi ${swToString(response)}`);

  response = await tx(reader, protocolRef, CIE_READ_NIS, "CIE READ NIS direct");
  if (!isSwOk(response))
    throw new Error(`CIE READ NIS direct ${swToString(response)}`);

  const data = response.slice(0, -2);
  const ascii = String.fromCharCode(...data);
  return /^[0-9]{8,20}$/.test(ascii) ? ascii : Array.from(data).join("");
}

async function tryReadCnsIdCarta(reader, protocolRef) {
  const SEL_MF_1 = Buffer.from([0x00, 0xa4, 0x00, 0x0c, 0x02, 0x3f, 0x00]);
  const SEL_MF_2 = Buffer.from([0x00, 0xa4, 0x00, 0x00, 0x02, 0x3f, 0x00]);
  const SEL_DF0_1 = Buffer.from([0x00, 0xa4, 0x00, 0x0c, 0x02, 0x10, 0x00]);
  const SEL_DF0_2 = Buffer.from([0x00, 0xa4, 0x00, 0x00, 0x02, 0x10, 0x00]);
  const SEL_EF_1 = Buffer.from([0x00, 0xa4, 0x00, 0x0c, 0x02, 0x10, 0x03]);
  const SEL_EF_2 = Buffer.from([0x00, 0xa4, 0x02, 0x0c, 0x02, 0x10, 0x03]);
  const SEL_EF_3 = Buffer.from([0x00, 0xa4, 0x00, 0x00, 0x02, 0x10, 0x03]);
  const READ16 = Buffer.from([0x00, 0xb0, 0x00, 0x00, 0x10]);

  let response;

  response = await tx(reader, protocolRef, SEL_MF_1, "CNS SELECT MF #1");
  if (!isSwOk(response)) {
    response = await tx(reader, protocolRef, SEL_MF_2, "CNS SELECT MF #2");
    if (!isSwOk(response))
      throw new Error(`SELECT MF failed (${swToString(response)})`);
  }

  response = await tx(reader, protocolRef, SEL_DF0_1, "CNS SELECT DF0 #1");
  if (!isSwOk(response)) {
    response = await tx(reader, protocolRef, SEL_DF0_2, "CNS SELECT DF0 #2");
    if (!isSwOk(response))
      throw new Error(`SELECT DF0 failed (${swToString(response)})`);
  }

  response = await tx(
    reader,
    protocolRef,
    SEL_EF_1,
    "CNS SELECT EF.ID_Carta #1",
  );
  if (!isSwOk(response))
    response = await tx(
      reader,
      protocolRef,
      SEL_EF_2,
      "CNS SELECT EF.ID_Carta #2",
    );
  if (!isSwOk(response))
    response = await tx(
      reader,
      protocolRef,
      SEL_EF_3,
      "CNS SELECT EF.ID_Carta #3",
    );
  if (!isSwOk(response))
    throw new Error(`SELECT EF.ID_Carta failed (${swToString(response)})`);

  response = await tx(reader, protocolRef, READ16, "CNS READ BINARY ID_Carta");
  if (!isSwOk(response))
    throw new Error(`READ ID_Carta ${swToString(response)}`);

  return String.fromCharCode(...response.slice(0, -2));
}

async function tryReadCnsContactless(reader, protocolRef) {
  const idCarta = await tryReadCnsIdCarta(reader, protocolRef);
  return {
    kind: "CNS",
    uuid: idCarta,
    source: "EF.ID_Carta",
  };
}

async function tryReadContactlessUid(reader, protocolRef) {
  const GET_UID = Buffer.from([0xff, 0xca, 0x00, 0x00, 0x00]);

  const response = await tx(
    reader,
    protocolRef,
    GET_UID,
    "CONTACTLESS GET UID",
  );
  if (!isSwOk(response)) throw new Error(`GET UID ${swToString(response)}`);

  const uid = response.slice(0, -2);
  if (!uid.length) throw new Error("GET UID returned no data");

  return uid;
}

async function tryReadNtag213(reader, protocolRef) {
  const READ_CC = Buffer.from([0xff, 0xb0, 0x00, 0x03, 0x04]);

  const uid = await tryReadContactlessUid(reader, protocolRef);
  if (uid.length !== 7)
    throw new Error(`NTAG213 UID length ${uid.length} not supported`);

  const response = await tx(reader, protocolRef, READ_CC, "NTAG213 READ CC");
  if (!isSwOk(response))
    throw new Error(`NTAG213 READ CC ${swToString(response)}`);

  const cc = response.slice(0, -2);
  if (cc.length < 4) throw new Error("NTAG213 READ CC returned short data");

  // Use only portable PC/SC probes: 7-byte UID plus the Type 2 CC size for NTAG213.
  if (cc[0] !== 0xe1 || cc[1] !== 0x10 || cc[2] !== 0x12) {
    throw new Error(`NTAG213 CC mismatch (${bufToHex(cc)})`);
  }

  return {
    kind: "NTAG213",
    uuid: bufToHexCompact(uid),
    source: "UID",
  };
}

function sendJson(ws, payload) {
  if (ws.readyState === WS_OPEN) {
    ws.send(JSON.stringify(payload));
  }
}

function broadcast(payload) {
  const message = JSON.stringify(payload);
  for (const client of wsClients) {
    if (client.readyState === WS_OPEN) {
      client.send(message);
    }
  }
}

function buildBaseEvent(type, details) {
  return {
    type,
    timestamp: new Date().toISOString(),
    ...details,
  };
}

function publishScanSuccess(details) {
  const payload = buildBaseEvent("scan", {
    success: true,
    ...details,
  });
  console.log(`[ws] scan success -> ${payload.uuid}`);
  broadcast(payload);
}

function publishScanError(details) {
  const payload = buildBaseEvent("scan", {
    success: false,
    ...details,
  });
  console.log(`[ws] scan error -> ${payload.error}`);
  broadcast(payload);
}

function serveIndex(res) {
  fs.readFile(INDEX_PATH, (err, data) => {
    if (err) {
      res.writeHead(500, { "Content-Type": "text/plain; charset=utf-8" });
      res.end("Failed to load demo page");
      return;
    }

    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
    res.end(data);
  });
}

const server = http.createServer((req, res) => {
  if (req.url === "/") {
    serveIndex(res);
    return;
  }

  if (req.url === "/health") {
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
    res.end(JSON.stringify({ ok: true, clients: wsClients.size }));
    return;
  }

  res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
  res.end("Not found");
});

const wss = new WebSocketServer({ noServer: true });

server.on("upgrade", (req, socket, head) => {
  if (req.url !== WS_PATH) {
    socket.destroy();
    return;
  }

  wss.handleUpgrade(req, socket, head, (ws) => {
    wss.emit("connection", ws, req);
  });
});

wss.on("connection", (ws) => {
  wsClients.add(ws);
  console.log(`[ws] client connected (${wsClients.size} total)`);

  sendJson(
    ws,
    buildBaseEvent("ready", {
      message: "websocket connected",
      wsPath: WS_PATH,
    }),
  );

  ws.on("close", () => {
    wsClients.delete(ws);
    console.log(`[ws] client disconnected (${wsClients.size} total)`);
  });

  ws.on("error", (err) => {
    console.error(`[ws] client error: ${err.message}`);
  });
});

pcsc.on("reader", (reader) => {
  console.log(`[pcsc] New reader: ${reader.name}`);
  readerBusy.set(reader.name, false);

  reader.on("error", (err) => {
    console.error(`[pcsc] Reader error (${reader.name}): ${err.message}`);
  });

  reader.on("end", () => {
    console.log(`[pcsc] Reader removed: ${reader.name}`);
    readerBusy.delete(reader.name);
  });

  reader.on("status", (status) => {
    const changes = reader.state ^ status.state;

    if (
      changes & reader.SCARD_STATE_PRESENT &&
      status.state & reader.SCARD_STATE_PRESENT
    ) {
      if (readerBusy.get(reader.name)) return;
      readerBusy.set(reader.name, true);

      const atrHexCompact = bufToHexCompact(status.atr);
      const atrPretty = bufToHex(status.atr);
      const mode = detectCardMode(reader.name, atrHexCompact);

      console.log(`\n[+] Card detected on ${reader.name}`);
      console.log(`    ATR: ${atrPretty || "n/a"}`);
      console.log(`    MODE: ${mode}`);

      connectReader(reader)
        .then(async (protocol) => {
          const protocolRef = { value: protocol };
          const basePayload = {
            reader: reader.name,
            mode,
            atr: atrPretty,
          };

          try {
            if (mode === "CIE_CONTACTLESS") {
              const nis = await tryReadCieNis(reader, protocolRef);
              const uuid = `CIE-${nis}`;
              console.log(`FINAL UUID: ${uuid}`);
              publishScanSuccess({
                ...basePayload,
                kind: "CIE",
                source: "EF.ID_Servizi",
                uuid,
              });
              return;
            }

            if (mode === "CNS_CONTACT") {
              const idCarta = await tryReadCnsIdCarta(reader, protocolRef);
              const uuid = `CNS-${idCarta}`;
              console.log(`FINAL UUID: ${uuid}`);
              publishScanSuccess({
                ...basePayload,
                kind: "CNS",
                source: "EF.ID_Carta",
                uuid,
              });
              return;
            }

            if (mode === "CNS_CONTACTLESS") {
              const result = await tryReadCnsContactless(reader, protocolRef);
              const uuid = `${result.kind}-${result.uuid}`;
              console.log(`FINAL UUID: ${uuid}`);
              console.log(`UUID SOURCE: ${result.source}`);
              publishScanSuccess({
                ...basePayload,
                kind: result.kind,
                source: result.source,
                uuid,
              });
              return;
            }

            if (mode === "UNKNOWN_CONTACTLESS") {
              try {
                const nis = await tryReadCieNis(reader, protocolRef);
                const uuid = `CIE-${nis}`;
                console.log(`FINAL UUID: ${uuid}`);
                publishScanSuccess({
                  ...basePayload,
                  kind: "CIE",
                  source: "EF.ID_Servizi",
                  uuid,
                });
                return;
              } catch (_err) {
                try {
                  const result = await tryReadNtag213(reader, protocolRef);
                  const uuid = `${result.kind}-${result.uuid}`;
                  console.log(`FINAL UUID: ${uuid}`);
                  console.log(`UUID SOURCE: ${result.source}`);
                  publishScanSuccess({
                    ...basePayload,
                    kind: result.kind,
                    source: result.source,
                    uuid,
                  });
                  return;
                } catch (_ntagErr) {
                  const uuid = `FP-${sha256Hex(`UNKNOWN_CONTACTLESS|${atrHexCompact}`).slice(0, 24)}`;
                  console.log(`FINAL UUID: ${uuid}`);
                  console.log("UUID SOURCE: UNKNOWN_CONTACTLESS_FINGERPRINT");
                  publishScanSuccess({
                    ...basePayload,
                    kind: "FP",
                    source: "UNKNOWN_CONTACTLESS_FINGERPRINT",
                    uuid,
                  });
                  return;
                }
              }
            }

            if (mode === "UNKNOWN_CONTACT") {
              try {
                const idCarta = await tryReadCnsIdCarta(reader, protocolRef);
                const uuid = `CNS-${idCarta}`;
                console.log(`FINAL UUID: ${uuid}`);
                publishScanSuccess({
                  ...basePayload,
                  kind: "CNS",
                  source: "EF.ID_Carta",
                  uuid,
                });
                return;
              } catch (_err) {
                const uuid = `FP-${sha256Hex(`UNKNOWN_CONTACT|${atrHexCompact}`).slice(0, 24)}`;
                console.log(`FINAL UUID: ${uuid}`);
                console.log("UUID SOURCE: UNKNOWN_CONTACT_FINGERPRINT");
                publishScanSuccess({
                  ...basePayload,
                  kind: "FP",
                  source: "UNKNOWN_CONTACT_FINGERPRINT",
                  uuid,
                });
                return;
              }
            }

            console.log("FINAL UUID: UNKNOWN");
            publishScanError({
              ...basePayload,
              error: "Unsupported card mode",
            });
          } catch (err) {
            console.log(`[debug] ${mode} failed: ${err.message}`);
            console.log("FINAL UUID: UNKNOWN");
            publishScanError({ ...basePayload, error: err.message });
          } finally {
            await disconnectReader(reader);
            readerBusy.set(reader.name, false);
          }
        })
        .catch((err) => {
          console.error(
            `[pcsc] connect error on ${reader.name}: ${err.message}`,
          );
          publishScanError({
            reader: reader.name,
            mode,
            atr: atrPretty,
            error: `connect error: ${err.message}`,
          });
          readerBusy.set(reader.name, false);
        });
    }

    if (
      changes & reader.SCARD_STATE_EMPTY &&
      status.state & reader.SCARD_STATE_EMPTY
    ) {
      console.log(`[pcsc] Card removed from ${reader.name}`);
    }
  });
});

pcsc.on("error", (err) => {
  console.error(`[pcsc] Global error: ${err.message}`);
});

server.listen(PORT, HOST, () => {
  console.log(`[server] Demo page: http://127.0.0.1:${PORT}/`);
  console.log(`[server] WebSocket: ws://127.0.0.1:${PORT}${WS_PATH}`);
});
